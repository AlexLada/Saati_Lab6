...
while True:
    try:
        num = int(input("Введите кол-во критериев: "))
        break
    except ValueError:
        print('Неверное значение')

s_matrica = np.eye(num)  # Создаём 1-ную матрицу
...
for i in range(a, num+1):
    for j in range(a+1, num+1):
        while True:
            try: # Заполняем каждый элемент строки матрицы
                s_matrica[i-1][j-1] = round(
                    float(input('Введите сравнение К{0}-К{1}:'.format(i, j))), 3)
                break
            except ValueError:
                print('Неверное значение')
        # Заполняем ячейки для обратного отношения (К1-К2 -> К2-К1)
        s_matrica[j-1][i-1] = round(s_matrica[i-1][j-1]**(-1), 2)
    a += 1

# Создаём список сумм строки
comp_list = [round(sum(j), 2) for j in s_matrica]
